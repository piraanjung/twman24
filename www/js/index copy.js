...
var app = {
	...
	connect: function () {
		...
		window.setTimeout(function () {
			...
			BTPrinter.connect(function (data) {
				$('#status').html('<span class="success">ทำการเชื่อมต่อบลูทูธเรียบร้อย</span>');
				// $('#printBase64_2').removeClass('hidden');
				// $('#btnConnect_div').removeClass('hidden');
				
			}, function (err) {
				$('#status').html('<span class="error"> ไม่สามารถเชื่อมต่อบลูทูธได้ </span>');
	...
	connected: function () {
		BTPrinter.connected(function (data) {
			...
			// $('#btnConnect_div').addClass('hidden');
			// $('#btnStatus_div').addClass('hidden');
			
			$('#status').html('<span class="success">connect:' + data + '</span>');
			// $('#printBase64_2').removeClass('hidden')
	...
	list: function () {
		BTPrinter.list(function (data) {
			...
			$('#status').html('<span class="success">เลือกชื่อเครืื่องปริ้นที่ต้องการเชื่อมต่อ แล้วกดปุ่ม <strong>เชื่อมต่อบลูธูท</strong></span>');
		}, function (err) {
			$('#status').html('<span class="error">list:' + err + '</span>');
	...
	printBarcode: function () {
		...
		BTPrinter.printBarcode(function (data) {
			$('#status').html('<span class="success">printBarcode' + data + '</span>');
		}, function (err) {
			$('#status').html('<span class="error">printBarcode' + err + '</span>');
	...
	printBase64: function () {
		...
		// var strAlign = $('#optAlign').val();
		BTPrinter.printBase64(function (data) {
			$('#status').html('<span class="success">' + data + '</span>');
		}, function (err) {
			$('#status').html('<span class="error">printBase64' + err + '</span>');
	...
	reset: function () {
		...
		var bytes = 'B140'; /* Initialize printer: https://reference.epson-biz.com/modules/ref_escpos/index.php?content_id=192 */
		$('#txtPOS').val(bytes);
		$('#btnPrintPOSCommand').click();
		// Reset GUI
	...
	status: function () {
		BTPrinter.status(function (data) {
			$('#status').html('<span class="success">เปิดใช้งานบลูธูทมือถือแล้ว</span>');
			// $('#connect_lists').removeClass('hidden')
		}, function (err) {
			$('#status').html('<span class="error">ยังไม่ได้เปิดใช้งานบลูธูทมือถือ</span>');
			// $('#connect_lists').addClass('hiden')
		});
	},
